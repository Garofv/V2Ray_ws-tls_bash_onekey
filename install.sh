...
#Move the configuration information of the old version to adapt to the version less than 1.1.0
...
chrony_install() {
    ...
    chronyc tracking -v
    date
    read -rp "Please make sure the time is correct, Error range ± 3 minutes(Y/N): " chrony_install
...
port_alterid_set() {
    if [[ "on" != "$old_config_status" ]]; then
        ...
        [[ -z ${port} ]] && port="443"
        read -rp "Please enter alterID（default:0 Only numbers are allowed）:" alterID
        [[ -z ${alterID} ]] && alterID="0"
    fi
}
modify_path() {
...
web_camouflage() {
    ##Please note that this conflicts with the default path of the LNMP script，Never use this script in an environment where LNMP is installed，otherwise at your own risk
    rm -rf /home/wwwroot
...
v2ray_install() {
    ...
    if [[ -f v2ray.sh ]]; then
        ...
        echo -e "${Error} ${RedBG} V2ray Installation file download failed，Please check if the download link is available ${Font}"
        exit 4
    fi
...
nginx_exist_check() {
    if [[ -f "/etc/nginx/sbin/nginx" ]]; then
        ...
        echo -e "${OK} ${GreenBG} Nginx installed by other packages detected，Continuing the installation will create a conflict，Please install after processing${Font}"
        exit 1
    else
        nginx_install
    fi
...
nginx_install() {
    ...
    echo -e "${OK} ${GreenBG} Compilation and installation are about to start Nginx, process a little longer，Please wait ${Font}"
    sleep 4
    ...
    # Add configuration folder，Adapt to legacy scripts
...
domain_check() {
    ...
    echo -e "${OK} ${GreenBG} Getting public ip information，Please wait ${Font}"
...
port_exist_check() {
    if [[ 0 -eq $(lsof -i:"$1" | grep -i -c "listen") ]]; then
        ...
        echo -e "${Error} ${RedBG} Detected that port $1 is occupied，The following is the occupancy information of $1 port ${Font}"
        lsof -i:"$1"
...
basic_information() {
    {
        ...
        echo -e "${Red} V2ray Successful installation ${Font}"
        echo -e "${Red} Domain（address）:${Font} $(info_extraction '\"add\"') "
        echo -e "${Red} Port（port）：${Font} $(info_extraction '\"port\"') "
        echo -e "${Red} userid（UUID）：${Font} $(info_extraction '\"id\"')"
        echo -e "${Red} extraid（alterId）：${Font} $(info_extraction '\"aid\"')"
